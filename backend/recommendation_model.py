...
def access_delay_minutes(defibrillator: Defibrillator) -> float:
...
def travel_metric_for(
    origin_lat: float,
    origin_lon: float,
    defibrillator: Defibrillator,
    travel_metrics: dict[str, TravelMetric] | None = None,
) -> TravelMetric:
    if travel_metrics and defibrillator.aed_id in travel_metrics:
        return travel_metrics[defibrillator.aed_id]

    distance = route_distance_m(origin_lat, origin_lon, defibrillator.lat, defibrillator.lon)
    one_way_seconds = distance / RUN_SPEED_MPS
    return TravelMetric(
        distance_m=distance,
...
def retrieval_minutes(
    defibrillator: Defibrillator,
    travel_metric: TravelMetric,
) -> float:
    return (travel_metric.round_trip_seconds / 60) + access_delay_minutes(defibrillator)
...
def best_backup_value(
    ...
    patient_to_primary = travel_metric_for(
        origin_lat,
        origin_lon,
        primary,
        travel_metrics,
    ...
    for backup in defibrillators:
        ...
        backup_to_patient = travel_metric_for(
            origin_lat,
            origin_lon,
            backup,
            travel_metrics,
        ...
        fallback_minutes = (
            ...
            + access_delay_minutes(primary)
            + access_delay_minutes(backup)
        )
        value = backup_reliability * time_weight(fallback_minutes)
...
def recommend_defibrillators(
    ...
    available_candidates = [
        (
            defibrillator,
            travel_metric_for(origin_lat, origin_lon, defibrillator, travel_metrics).distance_m,
        )
    ...
    for defibrillator, distance in primary_candidates:
        ...
        travel_metric = travel_metric_for(origin_lat, origin_lon, defibrillator, travel_metrics)
        minutes = retrieval_minutes(defibrillator, travel_metric)
        ...
        recommendations.append(
            Recommendation(
                ...
                one_way_seconds=round(travel_metric.one_way_seconds, 1),
                round_trip_seconds=round(travel_metric.round_trip_seconds, 1),
